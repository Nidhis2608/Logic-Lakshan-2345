...
const QuizStart = () => {
  ...
  const [totalScore, setTotalScore] = useState(0);
  const [userAnswers, setUserAnswers] = useState([]);
  const [userId, setUserId] = useState(""); // Add state to store user ID

  useEffect(() => {
    // Fetch user ID here (e.g., from localStorage or context)
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await axios.get(`${requrl}admin/quizzes/6609954e74458c636cfd0bea`);
        setQuiz(response.data);
        setQuestions(response.data.questions);
        await fetchQuestionsData(response.data.questions);
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(`${requrl}performance/submit`, {
        ...
        timeTaken: 0 // You can calculate time taken and send it here if needed
      });
    } catch (error) {
